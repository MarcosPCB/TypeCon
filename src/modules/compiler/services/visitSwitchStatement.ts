...
export function visitSwitchStatement(sw: SwitchStatement, context: CompilerContext) {
  let code = (context.options.lineDetail ? formatLineDetail(sw.getText()) : '') + visitExpression(sw.getExpression(), context);
  ...
  for (let i = 0; i < cases.length; i++) {
    ...
    if (c.isKind(SyntaxKind.DefaultClause))
      code += (context.options.lineDetail ? formatLineDetail(c.getText()) : '') + indent(`ifge rswc ${cases.length}\n  set rswe 1\n`, 1);
    else {
      const clause = c.getExpression();

      code += context.options.lineDetail ? formatLineDetail(c.getText()) : '';/* + indent(`state pushd\nstate pushc\n`, 1)*/
