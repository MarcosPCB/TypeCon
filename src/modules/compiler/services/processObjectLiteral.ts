import { ObjectLiteralExpression, SyntaxKind, PropertyAssignment, NumericLiteral, SpreadAssignment } from 'ts-morph';
  ...
  if (aliasName && context.typeAliases.has(aliasName)) {
    ...
    for (const [propName, propType] of Object.entries(typeDef.members)) {
      ...
      if (prop && prop.isKind(SyntaxKind.PropertyAssignment)) {
        ...
        // Property not provided: check spread arguments
        let found = false;
        for (const p of objLit.getProperties()) {
          if (p.isKind(SyntaxKind.SpreadAssignment)) {
            const spread = p as SpreadAssignment;
            ...
            if (spreadSym && spreadSym.children && spreadSym.children[propName]) {
              ...
              code += `add rsp 1\nsetarray flat[rsp] ra\n`;
              layout[propName] = { ...(child as any), offset: totalSlots, name: propName };
              found = true;
              break;
            }
          }
    ...
    for (const p of props) {
      ...
      if (p.isKind(SyntaxKind.PropertyAssignment)) {
        ...
        if (!spreadSym) {
          addDiagnostic(objLit, context, "error", `Spread argument ${spreadName} not found`);
          continue;
        }

        if (!spreadSym.children) {
          addDiagnostic(objLit, context, "error", `Spread argument ${spreadName} has no properties to spread`);
          continue;
        }

        for (const childKey in spreadSym.children) {
          const child = spreadSym.children[childKey] as SymbolDefinition;
          ...
          layout[childKey] = { ...(child as any), offset: totalSlots }; // Offset in the new object is simply the next slot
          totalSlots++;
        }
