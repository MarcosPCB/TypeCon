...
class TestPlayersNew extends CEvent {
    ...
    public Append(): void {
        ...
        // ── status sub-object ──────────────────────────────────────────────
        let onGnd: boolean = players[0].status.onGround;
        let jump: boolean = players[0].status.jumping;
        let crouch: boolean = players[0].status.crouching;
        let dead: boolean = players[0].status.dead;

        players[0].status.dead = false;
